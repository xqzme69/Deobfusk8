from __future__ import annotations
from typing import List

SBOX = [
    99,
    124,
    119,
    123,
    242,
    107,
    111,
    197,
    48,
    1,
    103,
    43,
    254,
    215,
    171,
    118,
    202,
    130,
    201,
    125,
    250,
    89,
    71,
    240,
    173,
    212,
    162,
    175,
    156,
    164,
    114,
    192,
    183,
    253,
    147,
    38,
    54,
    63,
    247,
    204,
    52,
    165,
    229,
    241,
    113,
    216,
    49,
    21,
    4,
    199,
    35,
    195,
    24,
    150,
    5,
    154,
    7,
    18,
    128,
    226,
    235,
    39,
    178,
    117,
    9,
    131,
    44,
    26,
    27,
    110,
    90,
    160,
    82,
    59,
    214,
    179,
    41,
    227,
    47,
    132,
    83,
    209,
    0,
    237,
    32,
    252,
    177,
    91,
    106,
    203,
    190,
    57,
    74,
    76,
    88,
    207,
    208,
    239,
    170,
    251,
    67,
    77,
    51,
    133,
    69,
    249,
    2,
    127,
    80,
    60,
    159,
    168,
    81,
    163,
    64,
    143,
    146,
    157,
    56,
    245,
    188,
    182,
    218,
    33,
    16,
    255,
    243,
    210,
    205,
    12,
    19,
    236,
    95,
    151,
    68,
    23,
    196,
    167,
    126,
    61,
    100,
    93,
    25,
    115,
    96,
    129,
    79,
    220,
    34,
    42,
    144,
    136,
    70,
    238,
    184,
    20,
    222,
    94,
    11,
    219,
    224,
    50,
    58,
    10,
    73,
    6,
    36,
    92,
    194,
    211,
    172,
    98,
    145,
    149,
    228,
    121,
    231,
    200,
    55,
    109,
    141,
    213,
    78,
    169,
    108,
    86,
    244,
    234,
    101,
    122,
    174,
    8,
    186,
    120,
    37,
    46,
    28,
    166,
    180,
    198,
    232,
    221,
    116,
    31,
    75,
    189,
    139,
    138,
    112,
    62,
    181,
    102,
    72,
    3,
    246,
    14,
    97,
    53,
    87,
    185,
    134,
    193,
    29,
    158,
    225,
    248,
    152,
    17,
    105,
    217,
    142,
    148,
    155,
    30,
    135,
    233,
    206,
    85,
    40,
    223,
    140,
    161,
    137,
    13,
    191,
    230,
    66,
    104,
    65,
    153,
    45,
    15,
    176,
    84,
    187,
    22,
]
INV_SBOX = [
    82,
    9,
    106,
    213,
    48,
    54,
    165,
    56,
    191,
    64,
    163,
    158,
    129,
    243,
    215,
    251,
    124,
    227,
    57,
    130,
    155,
    47,
    255,
    135,
    52,
    142,
    67,
    68,
    196,
    222,
    233,
    203,
    84,
    123,
    148,
    50,
    166,
    194,
    35,
    61,
    238,
    76,
    149,
    11,
    66,
    250,
    195,
    78,
    8,
    46,
    161,
    102,
    40,
    217,
    36,
    178,
    118,
    91,
    162,
    73,
    109,
    139,
    209,
    37,
    114,
    248,
    246,
    100,
    134,
    104,
    152,
    22,
    212,
    164,
    92,
    204,
    93,
    101,
    182,
    146,
    108,
    112,
    72,
    80,
    253,
    237,
    185,
    218,
    94,
    21,
    70,
    87,
    167,
    141,
    157,
    132,
    144,
    216,
    171,
    0,
    140,
    188,
    211,
    10,
    247,
    228,
    88,
    5,
    184,
    179,
    69,
    6,
    208,
    44,
    30,
    143,
    202,
    63,
    15,
    2,
    193,
    175,
    189,
    3,
    1,
    19,
    138,
    107,
    58,
    145,
    17,
    65,
    79,
    103,
    220,
    234,
    151,
    242,
    207,
    206,
    240,
    180,
    230,
    115,
    150,
    172,
    116,
    34,
    231,
    173,
    53,
    133,
    226,
    249,
    55,
    232,
    28,
    117,
    223,
    110,
    71,
    241,
    26,
    113,
    29,
    41,
    197,
    137,
    111,
    183,
    98,
    14,
    170,
    24,
    190,
    27,
    252,
    86,
    62,
    75,
    198,
    210,
    121,
    32,
    154,
    219,
    192,
    254,
    120,
    205,
    90,
    244,
    31,
    221,
    168,
    51,
    136,
    7,
    199,
    49,
    177,
    18,
    16,
    89,
    39,
    128,
    236,
    95,
    96,
    81,
    127,
    169,
    25,
    181,
    74,
    13,
    45,
    229,
    122,
    159,
    147,
    201,
    156,
    239,
    160,
    224,
    59,
    77,
    174,
    42,
    245,
    176,
    200,
    235,
    187,
    60,
    131,
    83,
    153,
    97,
    23,
    43,
    4,
    126,
    186,
    119,
    214,
    38,
    225,
    105,
    20,
    99,
    85,
    33,
    12,
    125,
]


def _gf_mult(a: int, b: int) -> int:
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi = a & 128
        a = a << 1 & 255
        if hi:
            a ^= 27
        b >>= 1
    return p


def _inv_shift_rows(s: List[int]) -> None:
    s[1], s[5], s[9], s[13] = (s[13], s[1], s[5], s[9])
    s[2], s[6], s[10], s[14] = (s[10], s[14], s[2], s[6])
    s[3], s[7], s[11], s[15] = (s[7], s[11], s[15], s[3])


def _inv_mix_columns(s: List[int]) -> None:
    for i in range(4):
        c = s[i * 4 : i * 4 + 4]
        s[i * 4] = (
            _gf_mult(14, c[0])
            ^ _gf_mult(11, c[1])
            ^ _gf_mult(13, c[2])
            ^ _gf_mult(9, c[3])
        )
        s[i * 4 + 1] = (
            _gf_mult(9, c[0])
            ^ _gf_mult(14, c[1])
            ^ _gf_mult(11, c[2])
            ^ _gf_mult(13, c[3])
        )
        s[i * 4 + 2] = (
            _gf_mult(13, c[0])
            ^ _gf_mult(9, c[1])
            ^ _gf_mult(14, c[2])
            ^ _gf_mult(11, c[3])
        )
        s[i * 4 + 3] = (
            _gf_mult(11, c[0])
            ^ _gf_mult(13, c[1])
            ^ _gf_mult(9, c[2])
            ^ _gf_mult(14, c[3])
        )


def _key_expansion(key: bytes) -> List[int]:
    rcon = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54]
    ks = list(key)
    for i in range(16, 176, 4):
        t = ks[i - 4 : i]
        if i % 16 == 0:
            t = [SBOX[t[1]], SBOX[t[2]], SBOX[t[3]], SBOX[t[0]]]
            t[0] ^= rcon[i // 16]
        for j in range(4):
            ks.append(ks[i - 16 + j] ^ t[j])
    return ks


def _decrypt_block(block: bytes, ks: List[int]) -> bytes:
    s = list(block)
    for i in range(16):
        s[i] ^= ks[10 * 16 + i]
    for r in range(9, 0, -1):
        _inv_shift_rows(s)
        for i in range(16):
            s[i] = INV_SBOX[s[i]]
        for i in range(16):
            s[i] ^= ks[r * 16 + i]
        _inv_mix_columns(s)
    _inv_shift_rows(s)
    for i in range(16):
        s[i] = INV_SBOX[s[i]]
    for i in range(16):
        s[i] ^= ks[i]
    return bytes(s)


def decrypt_aes_ecb(ct: bytes, key: bytes) -> bytes:
    ks = _key_expansion(key)
    out = b""
    for i in range(0, len(ct), 16):
        out += _decrypt_block(ct[i : i + 16], ks)
    return out


def obfusk8_api_hash(name: str, hash_iv: int) -> int:
    h = hash_iv & 4294967295
    for ch in name:
        c = ord(ch)
        if ord("a") <= c <= ord("z"):
            c -= 32
        low_byte = h & 255 ^ c & 255
        sub = SBOX[low_byte]
        h = (h >> 8 | h << 24) & 4294967295
        h = (h ^ sub) & 4294967295
    return h
